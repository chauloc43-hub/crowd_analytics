...
class LineCrossingCounter:
    ...
    def update(self, active_track_ids: list[int], line_start: Point, line_end: Point) -> None:
        ...
        for track_id in active_track_ids:
            ...
            # Check segment intersection or line crossing between previous and current points
            intersects = _segments_intersect(previous, current, line_start, line_end)
            if not intersects:
                # Fallback: check if previous and current points straddle the line
                cross_prev = _cross(line_start, line_end, previous)
                cross_curr = _cross(line_start, line_end, current)
                if (cross_prev * cross_curr) <= 0 and abs(cross_prev - cross_curr) > 1e-3:
                    intersects = True
